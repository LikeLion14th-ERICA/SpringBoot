...
/**
 * 보너스 과제 통합 - AppConfig를 통한 의존성 조립
 *
 * 저장소 타입을 변경하면 다른 구현체로 쉽게 교체할 수 있다.
 * - MEMORY: 메모리 저장소 (프로그램 종료 시 데이터 사라짐)
 * - FILE: 파일 저장소 (프로그램 종료 후에도 데이터 유지)
 * - MOCK: 테스트용 Mock 저장소 (고정된 더미 데이터)
 */
public class Main {
    public static void main(String[] args) {
        ...
        // ======= 저장소 타입 선택 =======
        System.out.println("🔧 저장소 타입을 선택하세요:");
        System.out.println("1. 메모리 저장소 (기본)");
        System.out.println("2. 파일 저장소 (데이터 유지)");
        System.out.println("3. Mock 저장소 (테스트용)");
        System.out.print("선택: ");
        int repoChoice = scanner.nextInt();
        scanner.nextLine();

        AppConfig.RepositoryType repoType;
        switch (repoChoice) {
            case 2 -> repoType = AppConfig.RepositoryType.FILE;
            case 3 -> repoType = AppConfig.RepositoryType.MOCK;
            default -> repoType = AppConfig.RepositoryType.MEMORY;
        }

        // ======= AppConfig를 통한 의존성 조립 =======
        AppConfig appConfig = new AppConfig(repoType);
    ...
    private static void printMenu() {
        System.out.println("🦁 ===== 멋사 멤버 관리 시스템 (보너스) ===== 🦁");
